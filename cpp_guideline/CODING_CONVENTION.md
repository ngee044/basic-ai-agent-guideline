...
## 16. MSA 확장(마이크로서비스) [선택]

시스템을 여러 서비스로 나누는 프로젝트에만 적용합니다(단일 프로세스면 생략). 아래는 CppToolkit 관용구로 구체화한 규칙이며, 채택 기준·패턴의 상세는 `msa_guideline/MSA_ARCHITECTURE.md`를 참조합니다. 기존 §7(오류)·§10(수명주기)·§11(동시성)·§12(로깅) 규약을 그대로 재사용합니다.

### 16.1 서비스 경계

- **1 실행 파일/프로세스 = 1 서비스**(1 bounded context)입니다. 엔티티/CRUD 단위로 잘게 쪼개지 않습니다(과분해 금지).
- 서비스 간 결합은 **메시지/API 계약으로만** 합니다. 다른 서비스의 내부 헤더를 직접 `#include`하거나 CMake 타겟으로 링크하지 않습니다(공유하는 것은 계약 스키마뿐).
- 각 서비스는 자기 데이터스토어(`{{DB}}`)만 소유합니다. 다른 서비스의 DB에 직접 접근하지 않고 API/이벤트로만 조회합니다(공유 DB 금지).

### 16.2 비동기 통신

- 상태 전파·느슨한 결합에는 `{{BROKER}}`(RabbitMQ/Kafka) 모듈로 발행/구독합니다. 서비스 간 결합을 줄이기 위해 가능하면 async를 선호합니다.
- 메시지는 공통 **envelope**로 감싸 `correlation_id`/`trace_id`를 포함시키고, 이 필드를 서비스 경계를 넘어 전파합니다(§16.5).
- 소비자 콜백은 기존 콜백 반환 규약(`std::expected<void, std::string>` 또는 애플리케이션 계층의 `std::tuple<bool, std::optional<std::string>>`, §7.4 각주)을 그대로 따릅니다.
- 소비자는 **idempotent**해야 합니다(at-least-once 전달 → 중복 delivery 가능). dedup 키는 **메시지 단위 고유 ID(`message_id` 또는 idempotency key)** 를 사용해 처리 이력을 확인합니다. `correlation_id`는 여러 메시지에 걸친 요청 흐름 상관/추적용이므로 중복 방어 키로 쓰지 않습니다(동일 `correlation_id`가 서로 다른 메시지에 붙을 수 있음).

envelope 필드(스키마는 프로젝트 계약 문서에 고정):

| 필드 | 용도 |
|------|------|
| `message_id` | 메시지 단위 고유 ID(중복 delivery 방어용 dedup/idempotency 키) |
| `correlation_id` | 요청 흐름 단위 상관관계 ID(요청-응답·saga·로그 상관) |
| `trace_id` | 분산 추적 ID(서비스 경계 전파) |
| `message_type` | 라우팅/디스패치 키 |
| `payload` | 직렬화된 본문(JSON 등) |

### 16.3 동기 통신

- 즉시 응답이 필요하면 Boost.Asio 기반 요청/응답 또는 REST 게이트웨이를 경유합니다.
- **모든 원격 호출에 타임아웃**을 둡니다(예: `boost::asio::steady_timer`로 데드라인 설정). 타임아웃 없는 동기 호출 체인은 금지합니다.
- 비동기/네트워크 경로에서 예외를 던지지 않는 규약(§7.3)을 지킵니다. 원격 호출 실패는 예외가 아니라 반환값(`std::expected`/`tuple`)으로 표현합니다.

### 16.4 회복탄력성

- 일시적 실패는 재시도하되 **exponential backoff + jitter**로 제한하고, 반복 실패에는 **circuit breaker** 개념(연속 실패 임계 초과 시 일정 시간 호출 차단)을 애플리케이션 레벨에서 구현합니다(특정 외부 라이브러리를 강제하지 않습니다).
- 자원 격리가 필요하면 호출 대상별로 ThreadPool/커넥션을 분리(bulkhead)합니다.
- **graceful shutdown**은 수명주기 규약(§10, §11)을 재사용합니다: 진행 중 요청/메시지를 드레인한 뒤 `stop()`(모듈별 `wait_stop()` 유무 차이 유의) → `reset()` 순으로 종료합니다.

### 16.5 관측성

- `Logger`에 `correlation_id`/`trace_id`를 **구조적으로** 남깁니다(`std::format`으로 조립, §12). 요청/메시지 단위로 동일 ID가 로그에 이어지게 합니다.
- 서비스 경계를 넘을 때는 envelope(§16.2) 또는 요청 헤더로 `trace_id`를 전파(context propagation)합니다. 신규 진입점에서 ID가 없으면 생성합니다.

### 16.6 헬스체크/수명주기

- 서비스별로 **헬스/레디니스 신호**를 노출합니다: REST 게이트웨이가 붙은 서비스는 `/healthz`(liveness)·`/readyz`(readiness)를, 순수 컨슈머는 주기적 **heartbeat 메시지**로 대체합니다.
- 종료 시 명시적 종료 순서(§10)를 따릅니다: 새 요청/소비 중단 → 진행 중 작업 드레인 → `stop()`/`reset()`.

### 16.7 설정

- 컴포넌트별 JSON 설정(기존 서술)을 외부화하고, 서비스 주소·브로커 주소·DB 접속 정보를 **하드코딩하지 않습니다**. 12-factor에 따라 환경변수/설정으로 주입하며, 서비스 위치는 service discovery/`{{REGISTRY}}` 또는 오케스트레이터 DNS로 해결합니다.

### 16.8 MSA 리뷰 체크리스트 (부록 B에 더해 확인)

- [ ] 서비스 간 결합이 계약(메시지/API)뿐인가? 다른 서비스 내부 헤더를 직접 `#include`/링크하지 않았는가?
- [ ] 메시지 envelope에 `message_id`/`correlation_id`/`trace_id`가 있고, 소비자가 `message_id`(dedup 키)로 idempotent(중복 delivery 방어)한가?
- [ ] 모든 원격 호출에 타임아웃이 있고, 실패를 예외 대신 반환값으로 표현하는가?
- [ ] 재시도에 backoff+jitter, 반복 실패에 circuit breaker 개념이 적용되는가?
- [ ] 로그에 `trace_id`가 구조적으로 남고 서비스 경계로 전파되는가?
- [ ] 헬스/레디니스 신호와 graceful shutdown(진행 중 작업 드레인)이 있는가?
- [ ] 서비스/브로커/DB 주소가 하드코딩되지 않고 외부화되었는가?

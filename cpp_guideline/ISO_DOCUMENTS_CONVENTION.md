...
## 14. MSA(다중 서비스) 문서화 [선택]

시스템을 여러 서비스로 분리하는 프로젝트에 적용합니다(단일 프로세스면 §2~§13의 4종 세트만으로 충분). 기존 SRS/SAD/STP/RTM 세트를 **시스템 레벨 1세트 + 서비스별 세트**로 확장하고, 서비스 카탈로그와 계약(contract) 산출물을 추가합니다. 규약·패턴의 상세는 `msa_guideline/ISO_MSA_CONVENTION.md`를 참조하며, 여기서는 기존 ID 체계(`FR-{AREA}-NN` 등, §5)와의 정합만 규정합니다.

### 14.1 문서 세트 확장

- **(a) 시스템 레벨(ISO/IEC/IEEE 15288 맥락)**: 시스템 전체의 요구사항·서비스 분해·서비스 간 상호작용을 다루는 1세트를 둡니다. 소프트웨어 생애주기(12207)에 시스템 생애주기(15288)를 더해, 시스템 SRS/SAD가 개별 서비스로의 분해와 서비스 간 계약을 규정합니다.
- **(b) 서비스별 세트**: 각 서비스(`{{SERVICE_NAME}}`)는 자신의 SRS/SAD/STP/RTM(또는 경량 서브셋)을 가집니다. 서비스가 작으면 시스템 SAD의 하위 절로 통합할 수 있으나, 독립 배포·소유권이 갈리면 서비스별로 분리합니다.

| 레벨 | 문서 ID 패턴(예) | 대상 |
|------|-----------------|------|
| 시스템 | `{{PROJECT_PREFIX}}-SYS-SRS-001`, `{{PROJECT_PREFIX}}-SYS-SAD-001` | 시스템 요구사항·서비스 분해·토폴로지·계약 목록 |
| 서비스 | `{{PROJECT_PREFIX}}-{SVC}-SRS-001` 등 | 개별 서비스(`{{SERVICE_NAME}}`)의 요구사항·설계·테스트·추적 |

- `{SVC}`는 서비스 식별 코드입니다(예: `SRV`, `CON`, `API`). 기존 기능 영역 코드 `{AREA}`(§5)와 정합되게 정하며, 대개 서비스=영역으로 대응됩니다.

### 14.2 서비스 카탈로그

시스템 SAD에 **서비스 카탈로그** 절을 두어 서비스 인벤토리를 단일 출처로 관리합니다.

| 서비스 | bounded context | 소유 데이터(`{{DB}}`) | 노출 API/구독 이벤트 | 문서 세트 | 소유 팀 |
|--------|-----------------|----------------------|--------------------|-----------|--------|
| `{{SERVICE_NAME}}` | … | … | … | `{{PROJECT_PREFIX}}-{SVC}-*` | … |

### 14.3 SAD 컴포넌트 뷰 → C4 Container 뷰 확장

- §7의 컴포넌트 뷰(§7.3)를 **C4 Container 뷰**로 확장합니다: 서비스(컨테이너)·`{{BROKER}}`·데이터스토어(`{{DB}}`)·게이트웨이(`{{GATEWAY}}`)의 **토폴로지**와 통신 방식(동기: REST/Boost.Asio vs 비동기: `{{BROKER}}` 이벤트)을 표기합니다.
- 서비스 내부는 기존 컴포넌트 뷰(`CMP-*`)를 그대로 유지합니다. 즉 C4 Container(서비스 간) 위에 §7의 컴포넌트 뷰(서비스 내부)를 얹는 구조입니다.
- 프로세스 뷰(§7 4장)는 서비스별 프로세스/스레드 모델로 유지하되, 서비스 간 메시지 흐름(End-to-End)에 `correlation_id`/`trace_id` 전파 경로를 표기합니다.

### 14.4 메시지/이벤트 계약 산출물

서비스 간 결합은 계약으로만 이뤄지므로, 계약을 **버전 관리되는 산출물**로 문서화합니다(시스템 레벨 또는 계약 전용 문서).

| 계약 항목 | 규정 내용 |
|----------|----------|
| 큐/토픽명 | `{{BROKER}}` 큐·exchange·라우팅 키 또는 topic |
| content-type | 직렬화 형식(예: `application/json`), delivery mode(durable 등) |
| envelope 스키마 | `correlation_id`/`trace_id`/`message_type`/`payload` 등 공통 봉투 필드 |
| payload 스키마 | 메시지 타입별 본문 필드·타입·필수 여부(버전 포함) |
| 호환성 정책 | breaking change 금지 규칙, 버전 표기, consumer-driven contract test 대상 |

- 계약에는 `EVT-{AREA}-NN`(이벤트) / `CTR-{SVC}-NN`(계약) 형태의 ID를 부여해 RTM에서 추적합니다(기존 §5 ID 체계와 정합).

### 14.5 RTM의 분산 추적 확장

기존 전방 추적(§9.1)의 열을 서비스 경계를 넘도록 확장합니다: **요구사항 → 서비스 → 컴포넌트(`CMP-*`) → 계약/이벤트(`CTR-*`/`EVT-*`) → 테스트**.

| 요구사항 ID | 서비스 | 설계 컴포넌트 | 계약/이벤트 | 테스트 케이스 | 소스 파일 | 상태 |
|-------------|--------|--------------|------------|--------------|-----------|------|
| FR-MSG-01 | `{{SERVICE_NAME}}` | CMP-API-HDL | EVT-MSG-01, CTR-API-01 | IT-07 | `message_handler.go` | 구현/검증 |

- 시스템 레벨 RTM은 서비스 간(요구사항↔계약↔서비스) 추적을, 서비스 레벨 RTM은 서비스 내부(요구사항↔컴포넌트↔테스트) 추적을 담당합니다.
- 정합성 체크(§10)에 항목을 추가합니다: 모든 계약(`CTR-*`/`EVT-*`)이 발행 서비스와 구독 서비스 양쪽에 매핑되는가, consumer-driven contract test가 존재하는가, 서비스 경계를 넘는 `trace_id` 전파가 관측성 요구사항으로 존재하는가.
